...
(function () {
    ...
    var resourcesRoot = '../ReactDesktopTest.Resources/';
    var webRoot = 'wwwroot/';
    ...
    var winFormsAssemblyInfoPath = '../ReactDesktopTest.AppWinForms/Properties/AssemblyInfo.cs';
    ...
    gulp.task('www-copy-resources-lib', function() {
        return gulp.src('../ReactDesktopTest.Resources/bin/Release/ReactDesktopTest.Resources.dll')
    ...
    gulp.task('www-msbuild-web', function() {
        return gulp.src('./ReactDesktopTest.csproj')
            .pipe(msbuild({
    ...
    gulp.task('www-msbuild-resources', function () {
        return gulp.src('../ReactDesktopTest.Resources/ReactDesktopTest.Resources.csproj')
            .pipe(msbuild({
                targets: ['Clean', 'Rebuild'],
                properties: {
                    Configuration: 'Release'
    ...
    gulp.task('www-msbuild-console', function () {
        return gulp.src('../ReactDesktopTest.AppConsole/ReactDesktopTest.AppConsole.csproj')
            .pipe(msbuild({
                targets: ['Clean', 'Rebuild'],
                properties: {
                    Configuration: 'Release'
    ...
    gulp.task('www-msbuild-winforms', function () {
        return gulp.src('../ReactDesktopTest.AppWinforms/ReactDesktopTest.AppWinforms.csproj')
            .pipe(msbuild({
                targets: ['Clean', 'Rebuild'],
                properties: {
                    Configuration: 'Release'
    ...
    gulp.task('www-nuget-restore', function() {
        return gulp.src('../../ReactDesktopTest.sln')
            .pipe(nugetRestore());
    });
...
gulp.task('www-nuget-pack-winforms', function (callback) {
        ...
        var excludes = globule.find([
            '../ReactDesktopTest.AppWinForms/bin/x86/Release/*.pdb',
            '../ReactDesktopTest.AppWinForms/bin/x86/Release/*.vshost.*'
        ]);
        var includes = [
        { src: '../ReactDesktopTest.AppWinForms/bin/x86/Release/*.*', dest: '/lib/net45/' },
        { src: '../ReactDesktopTest.AppWinForms/bin/x86/Release/locales/*.*', dest: '/lib/net45/locales/' }];
        return nugetpack({
            id: title,
            title: title,
                version: version,
